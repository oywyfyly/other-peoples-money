...
async function fetchData() {
    ...
    const getAi = await fetch("http://localhost:3000/getAi", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({ data: globalData })
    })
    globalDataComplete = await getAi.json()
    console.log(globalDataComplete)
}
...
document.getElementById('sort').addEventListener("click", (event) => {
    if (event.target.tagName === "BUTTON") {
        ...
        if (sortKey === 'Revenue' || sortKey === 'Profit' || sortKey === 'Employees') {
            ...
            const sortedGlobalData = [...globalDataComplete].sort((a, b) => {
            ...
            loadTableGlobal(globalDataComplete, sortKey);
        }
    }
});
...
document.getElementById('global').addEventListener("click", (event) => {
    ...
    loadTableGlobal(globalDataComplete, 'Valuation');
